...
dataPath <- "C:\\GitRepos\\Stat380FinalProjectSepichHainley\\GDP\\"
```

```{r}
files <- list.files(path = dataPath,
                    pattern = "^SAGDP2(N|S)__ALL_AREAS_[0-9]*_[0-9]*.csv$")
# SAGDP2
current <- fread(file = paste0(dataPath,files[1]))
head(current)
current <- current %>%
    left_join(fread(file = paste0(dataPath,files[2])),
              by = c("GeoFIPS","Region","GeoName","TableName","ComponentName",
                     "Unit","IndustryId","IndustryClassification","Description"))
write.csv(current, file = paste0(dataPath,"SAGDP2.csv"))
```

```{r}
files <- list.files(path = dataPath,
                    pattern = "^SAGDP4(N|S)__ALL_AREAS_[0-9]*_[0-9]*.csv$")
# SAGDP4
...
```{r}
files <- list.files(path = dataPath,
                    pattern = "^SAGDP9(N|S)__ALL_AREAS_[0-9]*_[0-9]*.csv$")
# SAGDP9
real <- bigBoii %>%
...
```{r}
files <- list.files(path = dataPath,
                    pattern = "^SAGDP10(N|S)__ALL_AREAS_[0-9]*_[0-9]*.csv$")
# SAGDP10
perCapita <- bigBoii %>%
