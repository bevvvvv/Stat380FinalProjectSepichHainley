...
# SAGDP2
current <- fread(file = paste0(dataPath,files[1]), fill = TRUE)
current <- current %>%
  select(-c(TableName))
head(current)
earlier <- fread(file = paste0(dataPath,files[2]), fill = TRUE) %>%
  select(-c(TableName, "1997"))
head(earlier)
blah <- current %>%
    left_join(earlier,
              by = c("GeoFIPS","Region","GeoName","ComponentName",
