...
```{r}
getGDPNumbers <- function(x) {
  cols <- grep("^(GDP:|GeoName)", names(x), value=TRUE)
  return(x[, cols])
}
```

```{r}
getImmigrationNumbers <- function(x) {
  cols <- grep("^(Immigrants:|GeoName)", names(x), value=TRUE)
  return(x[, cols])
}
```
...
createPopulationTable <- function() {
  ...
  real <- getGDPNumbers(real)
  per <- getGDPNumbers(per)
  pop <- real
  cols <- names(per)
...
perCapitaGDP <- compiledData %>%
  filter(TableName == "SAGDP10N")
outputPerWorker <- getGDPNumbers(perCapitaGDP)
